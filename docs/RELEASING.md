...
`gotth-jobs` versions independently under Semantic Versioning. A version number
...
No release may be tagged until required verification and a real consumer pin
are complete. The repository uses the MIT license.
...
5. Verify exact head and tag parity at
   `github.com/gotthboard/gotth-jobs` through the existing one-way push
   mirror.
