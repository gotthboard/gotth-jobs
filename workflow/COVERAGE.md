...
| JOB-010 | worker | `Worker.Run` | half-lease boundary, success, bounded handler errors, retry, panic, cancellation, heartbeat, reconciliation, PostgreSQL | covered |
...
Unit statement coverage is 97.5% at exact implementation source `b54c0fc`;
`boundedFailure` is 100% covered. The unchanged PostgreSQL integration suite
remains 97.4% ancestor evidence at `4ec1970` and was not rerun for a private,
database-independent Worker helper. Exact unrelated residual blocks are
